...
def generate_reply(
    ...
    if mode == "INTELLIGENCE_EXTRACTION":
        ...
        # ✅ HARD LOCK: OTP stage only (progressive)
        if stage == "OTP_FRAUD":
            return {
        ...
        # ✅ If bank/IFSC already present, strengthen bank intelligence
        ...
        # social/urgency + link missing => ask link
        if gaps["need_link"] and stage in ["SOCIAL_ENGINEERING", "URGENCY"]:
            return {"agentReply": _pick(ask_link, rng), "agentGoal": "Extract phishing URL for reporting."}

        # ✅ UPI exists => confirm receiver/collect/alt
        if gaps["has_upi"]:
        ...
        # payment request but no UPI yet => ask UPI
        ...
        # QR intent with UPI already => receiver/collect flow
